...
static void pong_actor(void *arg) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // Send pong back
        pm_copy.count++;
        status = rt_ipc_notify(ping_id, &pm_copy, sizeof(ping_msg));
...
static void ping_actor(void *arg) {
    ...
    // Send first ping
    ping_msg pm = { .count = 0 };
    rt_status status = rt_ipc_notify(pong_id, &pm, sizeof(ping_msg));
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // Send ping back
        recv_pm.count++;
        status = rt_ipc_notify(pong_id, &recv_pm, sizeof(ping_msg));

...
acrt_status acrt_file_init(void);
void acrt_file_cleanup(void);
...
acrt_status acrt_file_init(void) {
    ACRT_INIT_GUARD(g_file.initialized);
...
void acrt_file_cleanup(void) {
    ACRT_CLEANUP_GUARD(g_file.initialized);
...
acrt_status acrt_file_open(const char *path, int flags, int mode, int *fd_out) {
    ...
    if (!g_file.initialized) {
        return ACRT_ERROR(ACRT_ERR_INVALID, "File I/O subsystem not initialized");
    }

...
static timer_entry g_timer_pool[ACRT_TIMER_ENTRY_POOL_SIZE];
static bool g_timer_used[ACRT_TIMER_ENTRY_POOL_SIZE];
static acrt_pool g_timer_pool_mgr;
...
acrt_status acrt_timer_init(void);
void acrt_timer_cleanup(void);

// Handle timer event from scheduler (called when timerfd fires)
void acrt_timer_handle_event(io_source *source) {
    ...
    // Get the actor
    actor *a = acrt_actor_get(entry->owner);
    if (!a) {
        // Actor is dead - cleanup timer
        timer_close_fd(entry);
        SLIST_REMOVE(g_timer.timers, entry);
        acrt_pool_free(&g_timer_pool_mgr, entry);
        return;
    }

    // Send timer tick message to actor
    // Use ACRT_MSG_TIMER class with timer_id as tag, sender is the owning actor
    // No payload needed - timer_id is encoded in the tag
    acrt_status status = acrt_ipc_notify_ex(entry->owner, entry->owner, ACRT_MSG_TIMER,
                                       entry->id, NULL, 0);
    if (ACRT_FAILED(status)) {
        ACRT_LOG_ERROR("Failed to send timer tick: %s", status.msg);
...
acrt_status acrt_timer_init(void) {
    ACRT_INIT_GUARD(g_timer.initialized);

    // Initialize timer entry pool
    acrt_pool_init(&g_timer_pool_mgr, g_timer_pool, g_timer_used,
                 sizeof(timer_entry), ACRT_TIMER_ENTRY_POOL_SIZE);
...
void acrt_timer_cleanup(void) {
    ACRT_CLEANUP_GUARD(g_timer.initialized);
...
static acrt_status create_timer(uint32_t interval_us, bool periodic, timer_id *out) {
    if (!out) {
        return ACRT_ERROR(ACRT_ERR_INVALID, "Invalid arguments");
    }

    if (!g_timer.initialized) {
        return ACRT_ERROR(ACRT_ERR_INVALID, "Timer subsystem not initialized");
    }
    ...
    if (interval_us == 0) {
        ...
        its.it_value.tv_sec = interval_us / ACRT_USEC_PER_SEC;
        its.it_value.tv_nsec = (interval_us % ACRT_USEC_PER_SEC) * 1000;
    }
    ...
    timer_entry *entry = acrt_pool_alloc(&g_timer_pool_mgr);
    if (!entry) {
        close(tfd);
        return ACRT_ERROR(ACRT_ERR_NOMEM, "Timer entry pool exhausted");
    }
...
acrt_status acrt_timer_cancel(timer_id id) {
    ...
    if (found) {
        ...
        acrt_pool_free(&g_timer_pool_mgr, found);
        return ACRT_SUCCESS;
    }

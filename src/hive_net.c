...
static hive_status try_or_epoll(int fd, uint32_t epoll_events, int operation,
    ...
    hive_status timeout_status = hive_mailbox_handle_timeout(current, timeout_timer, "Network I/O operation timed out");
...
hive_status hive_net_accept(int listen_fd, int *conn_fd_out, int32_t timeout_ms) {
    ...
    hive_status status = try_or_epoll(listen_fd, EPOLLIN, NET_OP_ACCEPT, NULL, 0, timeout_ms);
    if (HIVE_FAILED(status)) {
        return status;
    }
...
hive_status hive_net_connect(const char *ip, uint16_t port, int *fd_out, int32_t timeout_ms) {
    ...
    if (inet_pton(AF_INET, ip, &serv_addr.sin_addr) != 1) {
        return HIVE_ERROR(HIVE_ERR_INVALID, "Invalid IPv4 address (hostnames not supported)");
    }
    ...
    if (connect(fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        ...
        hive_status status = try_or_epoll(fd, EPOLLOUT, NET_OP_CONNECT, NULL, 0, timeout_ms);
        if (HIVE_FAILED(status)) {
            close(fd);
            return status;
        }
...
hive_status hive_net_recv(int fd, void *buf, size_t len, size_t *received, int32_t timeout_ms) {
    ...
    // Would block - register interest in epoll and yield
    hive_status status = try_or_epoll(fd, EPOLLIN, NET_OP_RECV, buf, len, timeout_ms);
    if (HIVE_FAILED(status)) {
        return status;
    }
...
hive_status hive_net_send(int fd, const void *buf, size_t len, size_t *sent, int32_t timeout_ms) {
    ...
    // Would block - register interest in epoll and yield
    hive_status status = try_or_epoll(fd, EPOLLOUT, NET_OP_SEND, (void *)buf, len, timeout_ms);
    if (HIVE_FAILED(status)) {
        return status;
    }

...
static void switch_actor_a(void *arg) {
    ...
    while (ctx->count < ctx->max_count) {
        // Send ping to B
        int msg = 1;
        rt_ipc_notify(ctx->partner, &msg, sizeof(msg));
...
static void switch_actor_b(void *arg) {
    ...
    while (ctx->count < ctx->max_count) {
        ...
        // Send pong back to A
        int reply = 2;
        rt_ipc_notify(ctx->partner, &reply, sizeof(reply));
...
static void ipc_sender(void *arg) {
    ...
    for (uint64_t i = 0; i < ctx->max_count; i++) {
        rt_ipc_notify(ctx->partner, buffer, ctx->msg_size);
...
static void ipc_receiver(void *arg) {
    ...
    for (uint64_t i = 0; i < ctx->max_count; i++) {
        ...
        // Send ack
        rt_ipc_notify(ctx->partner, &ack, sizeof(ack));
    }

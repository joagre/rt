#ifndef ACRT_INTERNAL_H
#define ACRT_INTERNAL_H
...
#define ACRT_INIT_GUARD(initialized_flag) \
    do { \
        if (initialized_flag) { \
            return ACRT_SUCCESS; \
        } \
...
#define ACRT_CLEANUP_GUARD(initialized_flag) \
    do { \
        if (!(initialized_flag)) { \
            return; \
        } \
...
#define ACRT_REQUIRE_ACTOR_CONTEXT() \
    do { \
        if (!acrt_actor_current()) { \
            return ACRT_ERROR(ACRT_ERR_INVALID, "Not called from actor context"); \
        } \
    } while(0)

// Internal helper functions (implemented in acrt_ipc.c)
...
// Used by: timer, link subsystems (via acrt_ipc_notify_ex)
...
// Returns ACRT_ERR_TIMEOUT if timeout occurred, otherwise cancels timer and returns ACRT_SUCCESS
...
// Actor crash handler (called when actor returns without acrt_exit)
// Sets ACRT_EXIT_CRASH and yields to scheduler - never returns

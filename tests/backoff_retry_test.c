...
void sender_actor(void *arg) {
    ...
    for (int i = 0; i < MESSAGES_TO_FILL_POOL; i++) {
        data++;
        rt_status status = rt_ipc_send(receiver, &data, sizeof(data), IPC_COPY);
    ...
    char start_cmd = 'S';
    rt_ipc_send(args->controller, &start_cmd, sizeof(start_cmd), IPC_COPY);
...
void controller_actor(void *arg) {
    ...
    if (*cmd == 'S') {
        ...
        rt_ipc_send(args->receiver, cmd, sizeof(*cmd), IPC_COPY);
    }
